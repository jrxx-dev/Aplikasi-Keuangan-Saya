...
import { startOfWeek, endOfWeek, addWeeks, subWeeks, format, isSameDay, parseISO } from "date-fns";
import { id } from "date-fns/locale";

interface AverageIncomeChartWidgetProps {
    data?: {
...
export function AverageIncomeChartWidget({ data }: AverageIncomeChartWidgetProps) {
    ...
    const weeklyData = useMemo(() => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const dayData = dailyHistory.find(d => d.date === dateStr && d.type === "income");

            days.push({
                date: dateStr,
                dayName: format(dayDate, "EEE", { locale: id }), // Mon, Tue, etc.
                fullDate: format(dayDate, "d MMM", { locale: id }),
                income: dayData ? dayData.total : 0
            });
        }
        return days;
        ...
        >
            <CardHeader className="pb-2 relative z-10 shrink-0">
                <div className="flex justify-between items-center">
                    <CardTitle className="text-sm font-medium text-muted-foreground">Grafik Pemasukan Harian</CardTitle>
                    <div className="flex items-center gap-1">
                        ...
                        <WidgetColorPicker widgetId="average-income" widgetName="Grafik Pemasukan" />
                    </div>
                </div>
            </CardHeader>
                                    ...
                                    return (
                                        <div className="rounded-lg border bg-background p-2 shadow-sm text-xs">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <span className="font-bold text-emerald-500">{formatCurrency(payload[0].value as number)}</span>
                                            </div>
                                        </div>
                                    )
                                }
            ...
            <CardFooter className="pt-0 text-[10px] text-muted-foreground flex justify-between items-center relative z-10 mt-auto shrink-0">
                <span>{weeklyData.reduce((acc, curr) => acc + curr.income, 0) > 0 ? `Total: ${formatCurrency(weeklyData.reduce((acc, curr) => acc + curr.income, 0))}` : "Tidak ada data"}</span>
            </CardFooter>
        </Card>
    );
}
